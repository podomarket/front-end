...
const addProduct = (payload) => {
  ...
  axios
    .post("http://3.86.227.196:8080/products", frm, {
      headers: {
...
const updateProduct = (payload) => {
  ...
  frm.append("file", payload.file);
  axios
    .put(`http://3.86.227.196:8080/products/${payload.id}`, frm, {
      headers: {
        Authorization: accessToken,
        "Refresh-Token": refreshToken,
        "Content-Type": "multipart/form-data",
...
export const __getProducts = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    try {
      const products = await axios.get("http://3.86.227.196:8080/products");
